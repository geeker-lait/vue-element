...
var baseConfig = {
    entry: {
        app: './src/main.js',
        'vendor': ['vue','vuex','vue-router','axios','babel-polyfill','element-ui']
    },
    output: { //配置打包结果， path 定义了输出的文件夹，filename则定义了打包结果文件的名称
        path: path.join(__dirname,"./dist"), // 输出文件的保存路径
        filename: statics + 'js/[name].js', //只要再加上hash这个参数就可以了
        publicPath : "/",
...
if(buildConfig.env === 'dev'){//开发环境
    ...
    webpackConfig = merge(baseConfig, {
        ...
        devServer : {
            ...
            proxy: {//本地代理
                '/userstest/*': {
                    target: 'http://101.200.42.194:9090/',
                    secure: false
                },
                '/user/*': {
                ...
                '/eagleeye/*': {
                    target: 'http://192.168.1.125:60008',
                    secure: false
                },
            }
        },
